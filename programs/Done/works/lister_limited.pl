...
#open(STDOUT, "| tee c:/seeii/log/lister.log STDOUT");
open(STDOUT, ">>c:/seeii/log/lister.log") or die "Cannot open LOGFILE for WRITING: $!";
open(STDERR, ">&STDOUT") or die "Cannot open LOGFILE for WRITING: $!";
  ...
  my $process_log = " Start - " . localtime() . "   Lister \n";
...
# accordingly in the READ_URL_TAB, and URL_REJECTED_TAB.
  ...
  my $select_url_list = $dbh->prepare_cached("SELECT URL, ACCEPT, FORCE_IND FROM URL_LIST_TAB LIMIT 250000");
  ...
  my $insert_reject = $dbh->prepare_cached("INSERT IGNORE INTO URL_REJECTED_TAB (URL) VALUES (?)");
  ...
  print localtime() . " - LISTER program started\n";
  my $pid = $$;
  ...
  print localtime() . " - LISTER program ended\n";

  my $process_log_file = "c:/seeii/process/log.txt";
  open(PLFILE, ">>$process_log_file") or warn "Cannot open $process_log_file for WRITING: $!";
  my $process_log = " Close - " . localtime() . "   Lister \n";
  print PLFILE $process_log;
  close(PLFILE);
...
sub process_list {
  ...
  my @row_url = ();
  my @row_read = ();
  my ($url_id, $url, $accept, $force, $donotprocess);
  ...
  while(@row_list = $select_url_list->fetchrow_array) {
    ...
    if ((($force eq 'Y') || ($accept eq 'Y')) && ($skip_ind eq 'n')) {
      $url_id = add_url($url);
      if ($url_id) {
