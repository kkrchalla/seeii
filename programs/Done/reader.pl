...
BEGIN {
  ...
  my $force_end_file = "c:/seeii/process/endreader";
...
# adds the domain name to the database
  ...
  my ($select_read, $select_url, $select_domain, $select_check_accept, $select_content);
...
#        3261 => '',         #  KANNADA SIGN AVAGRAHA
  ...
  $select_read   = $dbh->prepare_cached("SELECT A.URL_ID, DATE(A.ADD_DATE), B.URL, B.LAST_UPDATED_DATE, B.QUALIFIED_FOR_LINK FROM READ_URL_TAB A, URL_TAB B WHERE A.URL_ID = B.URL_ID AND A.QUEUE_NUM = ? ORDER BY 2, 3 LIMIT 4000");
  $delete_read   = $dbh->prepare_cached("DELETE FROM READ_URL_TAB WHERE URL_ID = ?");
  $update_url     = $dbh->prepare_cached("UPDATE URL_TAB SET SIZE = ?, LAST_UPDATED_DATE = ?, CONTENT_DIGEST_ID = ?, LAST_READ_DATE = NOW(), DOMAIN_ID = ? , QUALIFIED_FOR_LINK = ? WHERE URL_ID = ?");
  $insert_url    = $dbh->prepare_cached("INSERT IGNORE INTO URL_TAB (URL, QUALIFIED_FOR_LINK) VALUES (?, ?)");
  $select_url    = $dbh->prepare_cached("SELECT URL_ID, QUALIFIED_FOR_LINK FROM URL_TAB WHERE URL = ?");
  $select_domain = $dbh->prepare_cached("SELECT DOMAIN_ID, ACCEPT FROM DOMAIN_TAB WHERE DOMAIN = ?");
  $insert_domain = $dbh->prepare_cached("INSERT IGNORE INTO DOMAIN_TAB (DOMAIN, ACCEPT, COUNTRY) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE ACCEPT = VALUES(ACCEPT), COUNTRY = VALUES(COUNTRY)");
  ...
  open(PLFILE, ">>$process_log_file") or warn "Cannot open $process_log_file for WRITING: $!";
  $process_log = " Close - " . localtime() . "   Reader - $queue_num - $reader_count $premature_end $forced_end\n";
  print PLFILE $process_log;
  close(PLFILE);
...
sub process_read {
  ...
  while(@row_read = $select_read->fetchrow_array) {
    ...
    if (time() - $start_time > 21600) {
      print "Premature Ending due to the time limit of 6 hours. \n";
...
sub read_web {
	...
	if( $meta_tags eq "noindex" ) {	# indexing this file is forbidden by meta tags
		...
		if ($parent_url_status eq ' ') {
			$parent_url_status = $url_status;
		}

		$content = substr($content, 0, 49500);
		$success &&= $insert_content->execute($url_id, $digest, $content);
		$success &&= $update_url->execute($filesize, $date, $digest, $domain_id, $parent_url_status, $url_id);
...
sub check_accept_url {
	...
	($domain_id, $domain_accept) = init_robot_check($base_domain, $base_host);
...
sub init_robot_check {
	...
	if (!@row_domain) {
		...
		($domain_accept, $country) = get_domain_accept($base_host);
		$success &&= $insert_domain->execute($base, $domain_accept, $country);
...
## If domain had .in in the end, it is indian.
## If domain had indian keywords, it is indian.
## If domain is from India, it is indian.
sub get_domain_accept {
  my $base = shift;
  ...
  my $country = $gi->country_code_by_name($base);
  if ($country eq 'IN') {
     return ('Y', $country);
  }
  ...
  if (check_for_keywords($base) eq 'y') {
     return ('Y', $country);
  }
...
sub add_url {
  ...
  if (! @row_url) {
    my $url_status = ' ';
    if ((check_for_keywords($url)) eq 'y') {
       $url_status = 'C';
    } 
    $success &&= $insert_url->execute($url,$url_status);
  ...
  $return_string = $row_url[0] . $row_url[1];
...
sub check_for_keywords {
  ...
  foreach my $indian_word (@indian_array) {
    if ($string =~ m/$indian_word/igs) {
       return 'y';
    }
  }
