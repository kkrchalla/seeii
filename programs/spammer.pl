...
# This program will delete all the spammed urls that have null add_date.
# This program will delete the urls from all the tables.
# In the end, it will update the SPAM_URL_TAB with the current timestamp as the add date.
  ...
  my $select_spam  = $dbh->prepare_cached("SELECT URL FROM SPAM_URL_TAB WHERE ADD_DATE IS NULL");
  my $update_spam  = $dbh->prepare_cached("UPDATE SPAM_URL_TAB SET ADD_DATE = CURRENT TIMESTAMP WHERE URL = ?");
  ...
  print localtime() . " - SPAMMER program started\n";

  process_spam();

  $dbh->disconnect;

  print localtime() . " - SPAMMER program ended\n";
...
sub process_spam {
  ...
  # select rows from spam_url
  $select_spam->execute();
  # for each row from spam_url
  while(@row_spam = $select_spam->fetchrow_array) {
    $success = 1;
    $url = $row_spam[0];
    ...
    if ($row_url[0]) {
      ...
      $success &&= $update_spam->execute($url);
    }
    $result = ($success ? $dbh->commit : $dbh->rollback);
    unless ($result) { 
      print "Error committing changes to spammer URL: $url, URL-ID: $url_id \n";
    }

  }
  if ($select_spam->rows == 0) {
    print "No entries in the SPAM_URL_TAB\n\n";
  }
  $select_spam->finish;
